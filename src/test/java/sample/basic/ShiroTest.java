...
public class ShiroTest {
   ...
   public int getMaximumEvents(int[] payload) {
      ...
      // Populate frequency map and find min/max values
      ...
      // Add days for minimum value
      ans += Math.min(freq.get(minValue), 2);

      // Add days for maximum value
      ans += Math.min(freq.get(maxValue), 2);

      // Iterate through the map and calculate for remaining values
